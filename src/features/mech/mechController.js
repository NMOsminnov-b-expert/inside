...
import { OI_SUBTYPE } from '../oi/oiSubtypes.js';
  ...
  if (ma) {
    ma.onclick = () => {
      ...
      tb.insertAdjacentHTML('beforeend', `<tr>
        <td><input class="input" placeholder="Наименование ОИ"></td>
        <td>
          <select class="select">
            ${['Узел', 'Агрегат', 'Станция', 'Прочее'].map((o) => `<option>${o}</option>`).join('')}
          </select>
        </td>
        <td><input class="input" placeholder="Код ЕНИ"></td>
        <td><button class="btn btn-ghost btn-sm" data-mech-del>×</button></td>
      </tr>`);
    ...
    ms2.onclick = () => {
      ...
      toast(
        appState.mechMode === 'mono'
          ? 'Объект добавлен (монолит)'
          : 'Комплекс добавлен, ОИ создаются внутри',
        'ok'
      );
    };
  }
