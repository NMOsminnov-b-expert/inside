...
export function bindOc() {
  document.querySelectorAll('[data-tab]').forEach((b) => b.onclick = () => {
    ...
    if (appState.tab === 'docs') {
      ...
      if (!appState.viewerDoc && DOCS.length) {
        appState.viewerDoc = {
          scope: 'oc',
          id: DOCS[0].id,
        };
      ...
      if (appState.viewer && appState.viewer.mode !== 'photo') {
  ...
  const bd = $('#btnDelOc');
  if (bd) {
    bd.onclick = () => toast('Удаление ОЦ — с подтверждением и проверкой связей', 'warn');
  }
  ...
  document.querySelectorAll('[data-doc-del]').forEach((b) => b.onclick = (e) => {
    ...
    if (appState.viewerDoc && appState.viewerDoc.id === id) {
      appState.viewerDoc = VS.openTabs.oc.length
        ? {
          scope: 'oc',
          id: VS.openTabs.oc[VS.openTabs.oc.length - 1],
        }
        : null;

      if (!appState.viewerDoc) {
        appState.viewer = null;
      }
    }
  ...
  if (appState.view === 'ocform') {
    const form = getOcFormDefinition(OC);

    if (form && typeof form.bind === 'function') {
      form.bind(OC);
    }
  }
}