...
export function oiLabel(oi) {
  ...
  // Механизмы и оборудование (гражданское здание): подпись уже собрана из
  // состава единиц, повторять вид в ней незачем.
  if (oi.card === 'mech') {
    return ['Механизмы и оборудование', name].filter(Boolean).join(' · ');
  }
  if (oi.card === 'movable') {
