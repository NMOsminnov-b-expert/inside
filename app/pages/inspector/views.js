...
// показывает то, что дали tasks.js и запись.
//
// Почему отдельные экраны, а не адаптив настольных (решение пользователя
// 08.09.2026): у осмотрщика телефон, одна рука и улица. Настольная карточка ОЦ
// — это широкие таблицы, просмотрщик документов рядом с полями и панель
// фильтров; ужать это до 390 пикселей нельзя без того, чтобы всё стало
// нажимаемым только ногтем.
import { esc } from '../../kernel/dom.js';
import { fmtEni } from '../../kernel/fmt.js';
import { PHOTO_CATS, PHOTO_LIMIT, READY_STATUS } from './tasks.js';
...
// Нижняя панель разделов. Четыре пункта — предел, за которым подписи перестают
// читаться на узком экране; пятый раздел пришлось бы прятать в «ещё», а это
// уже не «быстро и удобно».
const TABS = [
...
// --- экран 3: карточка ОЦ под телефон -------------------------------------

export function objectHTML({ rec, openOi, hrefFor, counts, backHref }) {
  const oi = rec.oi || [];
  ...
  return `<div class="ins">
    ...
    <div class="ins-body">
      <p class="ins-ro">${ico('ok', 14)} Только для просмотра: правки вносит ЦОД</p>
      ...
      <section class="ins-blk">
        ...
        ${oi.length ? `<ul class="ins-oi">
          ${oi.map((o) => oiItemHTML(o, o.id === openOi)).join('')}
...
// Литеры раскрываются по одной: на телефоне десять развёрнутых карточек — это
// сплошная прокрутка, в которой не найти нужную.
function oiItemHTML(o, open) {
  const head = [o.letter ? 'Литера ' + o.letter : '', o.name].filter(Boolean).join(' · ');
  ...
  return `<li class="ins-oi-i ${open ? 'on' : ''}">
    ...
    ${open ? `<div class="ins-oi-b">
      ...
      ${rowHTML('Фундамент', (o.struct || {}).foundation)}
      ${rowHTML('Наружные стены', (o.struct || {}).wallsExt)}
      ${rowHTML('Кровля', (o.struct || {}).roof)}
    </div>` : ''}
  </li>`;
}
...
function docItemHTML({ doc: d, owner }) {
  ...
  const meta = [owner, d.type, d.date, file && file.name].filter(Boolean).join(' · ');
...
export function photoHTML({ rec, groups, total, cat, hrefFor, counts, backHref }) {
  return `<div class="ins">
    ${headerHTML({
    title: 'Фото с осмотра',
    sub: `${total} из ${PHOTO_LIMIT}`,
    backHref,
    backLabel: 'К задаче',
  })}
    ...
    <div class="ins-body">
      <section class="ins-blk">
        ...
        <button class="ins-btn ins-btn-main ins-btn-wide" data-shoot>${ico('photo')} Снять или выбрать</button>
        <p class="ins-hint">Снимок попадёт в выбранную категорию — потом его
          можно перенести в другую.</p>
      </section>

      ${total ? [...groups.entries()].map(([name, list]) => `<section class="ins-blk">
        <h2 class="ins-blk-h">${esc(name)} · ${list.length}</h2>
        <ul class="ins-grid">
          ${list.map((p) => `<li class="ins-shot">
            ...
            <span class="ins-shot-acts">
              <select class="ins-select ins-select-sm" data-move="${esc(p.id)}" title="Перенести в другую категорию">
                ${PHOTO_CATS.map((c) => `<option ${c === p.cat ? 'selected' : ''}>${esc(c)}</option>`).join('')}
              </select>
              <button class="ins-icon-b" data-drop="${esc(p.id)}" aria-label="Удалить снимок"
                title="Удалить снимок">${ico('trash', 18)}</button>
            </span>
          </li>`).join('')}
        </ul>
...
// Полноэкранный просмотр снимка. Отдельным слоем, а не переходом: осмотрщик
// смотрит снимок, чтобы решить «переснять или нет», и должен возвращаться в
// сетку одним касанием.
export function lightboxHTML(p) {
  return `<div class="ins-lb" data-lb>
    ...
    <span class="ins-lb-cap">${esc(p.cat)} · ${esc(p.at)}</span>
  </div>`;
}
