...
// Строка для поиска: нижний регистр и «ё» → «е».
//
// Пишут вперемешку — и в самих справочниках, и в строке поиска, — а человек
// ищет то, что помнит, не думая о двух точках. Без приведения поиск
// «перекрытие» не находил вариант с «ё», и это выглядело как «значения нет».
export const foldSearch = (s) => String(s ?? '').toLowerCase().replace(/ё/g, 'е');
...
//
// Длин у кода три: 13, 15 и 18 цифр (пользователь 04.09.2026 — раньше принималась
// только полная, 18, и короткие коды подсвечивались как ошибка). Это не три
// разных формата, а одна и та же маска разной глубины: 13 цифр — это первые пять
// групп (1-2-2-4-4), 15 — шесть (плюс NN), 18 — все семь (плюс YYY). Поэтому и
// маска, и определение области работают одинаково на любой из них, а списком
// длин остаётся ровно набор накопленных сумм групп — отдельно его держать не
// нужно, он выводится из ENI_GROUPS и не разъедется с маской.
//
// Код длины не из этого набора — по-прежнему ошибка данных, а не иной формат:
// показываем как есть и помечаем через eniError, а не подгоняем под маску молча.
// Могут ли коды ЕНИ повторяться (уточнение пользователя 03.09.2026: «бывает
// такое, что ЕНИ дублируются»). false — совпадения допустимы: записи с одним
// кодом различаются скрытым индексом (eniIndex), человеку он не показывается.
// true — код уникален: система не даёт создать вторую запись с тем же кодом, а
// архив держит бронь на коды своих записей.
//
// Флаг читают ровно три места: kernel/eniField.js (ввод и проверка),
// генераторы кода в модулях и возврат объекта из архива (kernel/archive.js).
...
// Полная длина — для подсказок и генераторов кода.
export const ENI_LENGTH = ENI_GROUPS.reduce((a, b) => a + b, 0);

// Допустимые длины: накопленные суммы групп, из которых код может обрываться.
// Обрыв на середине группы кодом не бывает, поэтому 14 или 16 — ошибка.
const ENI_STOPS = [5, 6, 7];
export const ENI_LENGTHS = ENI_STOPS.map(
  (n) => ENI_GROUPS.slice(0, n).reduce((a, b) => a + b, 0),
);
...
// длина которого не совпадает ни с одной из допустимых.
//
// В сообщении перечисляем все допустимые длины: человеку, набравшему 14 цифр,
// важно знать, дописать ему цифру или стереть, а не только что «не 18».
export function eniError(value) {
  const digits = String(value ?? '').replace(/\D/g, '');
  if (!digits) return '';
  if (!ENI_LENGTHS.includes(digits.length)) {
    // Склонение через общий plural: своя укороченная формула давала «3 цифр».
    const has = `${digits.length} ${plural(digits.length, 'цифра', 'цифры', 'цифр')}`;
    return `В коде ЕНИ ${has}, а должно быть ${ENI_LENGTHS.join(', ')}`;
  }
  return '';
}
