...
import { enhanceMsDrops } from './msDrop.js';
...
export function createScope(root) {
  ...
  const scope = {
    ...
    setHTML(html) {
      ...
      enhanceSelects(root);
      // Тем же одним касанием — всплытие открытых мультивыборов: иначе список
      // материалов обрезается ячейкой таблицы «Конструктив и износ» (см.
      // msDrop.js).
      enhanceMsDrops(root);
    },
