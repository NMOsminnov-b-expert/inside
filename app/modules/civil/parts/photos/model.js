...
// Подпись категории фото для показа. У большинства ОИ категория и есть подпись
// («Фасад», «Кровля»), а у механизмов категория — id единицы техники, и
// показывать надо её название. Названия лежат на самом ОИ (oi.photoCatNames,
// их держит в актуальном виде oi/mech/model.js) — не через реестр карточек:
// реестр подгружает карточки, а те сами пользуются этим файлом, и ссылка на
// реестр отсюда замкнула бы круг зависимостей.
export function catLabel(oi, cat) {
  return (oi && oi.photoCatNames && oi.photoCatNames[cat]) || cat;
}
