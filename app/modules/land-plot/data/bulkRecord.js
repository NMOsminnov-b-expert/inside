...
function landOi(id, i, p, suffix, opts = {}) {
  return {
    ...
    buildingType: '',
    buildingArea: '',
    gasification: i % 3 ? 'Проходят рядом' : 'Отсутствуют в районе',
    centralHeating: 'Отсутствуют в районе',
    centralWater: i % 2 ? 'Подведены' : 'Проходят рядом',
    autonomousHeating: 'На удалении',
    status: opts.status || 'Основное',
    origin: 'manual',
...
export function buildBulkRecord(id, i, p) {
  ...
  rec.oi.push(landOi(id, i, p, 1, {
    name: p.metrics.oiCount > 1 ? 'Участок №1 (основной)' : 'Земельный участок',
    area: p.metrics.area * 0.7,
  }));

  if (p.metrics.oiCount > 1) {
    rec.oi.push(landOi(id, i, p, 2, {
      name: 'Участок №2 (смежный)',
      area: p.metrics.area * 0.3,
