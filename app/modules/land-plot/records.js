...
// Тип земель записи: считается по её участкам и приписывается к типу ОЦ —
// «Земельный участок · с/х» (заметка Даниила, решения пользователя 05.09.2026).
//
// Только у типа ОЦ «Земельный участок»: у здания участок под ним — часть
// объекта, а не то, чем объект является, и приписка там читалась бы как
// характеристика самого здания (уточнение пользователя 05.09.2026).
function landKindOf(rec) {
  if (manifest.id !== 'land-plot') return '';

  const kinds = new Set((rec.oi || [])
    .filter((o) => o.card === 'land')
    .map((o) => (o.landType === 'Несельскохозяйственный' ? 'не с/х' : 'с/х')));

  if (!kinds.size) return '';
  if (kinds.size > 1) return 'смеш.';
  return [...kinds][0];
}
...
export function summarize(rec) {
  ...
  return {
    ...
    facts: [
      ...
      { label: 'Площадь участков', value: m.area ? fmtNum(m.area) + ' м²' : '—' },
