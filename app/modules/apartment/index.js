...
import { migrateAnnexList } from './oi/apartment/annexes.js';
...
export function main(host) {
  ...
  function migrateAnnexes(r) {
    ...
    r.oi.forEach((o) => {
      ...
      // Три списка (лоджии, балконы, террасы) собираются в одну таблицу
      // пристроек с литерой, видом и материалами — как в карточке гражданского
      // здания (указание пользователя 10.09.2026). Перевод идёт ПОСЛЕ
      // migrateAreaList: тот приводит старые счётчики к спискам, а этот
      // собирает списки в таблицу.
      if (o.card === 'apartment') migrateAnnexList(o);
    });
  }
