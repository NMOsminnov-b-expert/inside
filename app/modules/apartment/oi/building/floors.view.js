...
function mansardTypeField(oi) {
  return `<div class="field" style="max-width:220px;margin-bottom:8px"><label>Конструктивный тип мансарды</label>
<select class="select" data-mansard-type>${MANSARD_TYPE.map((o) => `<option ${o === (oi.mansardType || MANSARD_TYPE[0]) ? 'selected' : ''}>${o}</option>`).join('')}</select></div>`;
}
...
${cat.key === 'mansard' ? mansardTypeField(oi) : ''}
