...
// у них поле. Распределяется только площадь по внутреннему обмеру: она и есть
// сумма этажей. Площадь по внешним замерам этажи не делят — её вводят руками
// у каждой строки (решение пользователя 11.09.2026; до него распределялась
// внешняя — почему поменяли, сказано ниже).
//
...
// складывается — и довод вместе с подписью потерял силу. У литеры ровно так же:
// сначала правило применили к квартире, потом пользователь распространил его на
// все карточки.
// Порядок колонок: сначала внутренний обмер, потом внешние замеры (решение
// пользователя 11.09.2026). Считают по внутреннему — он и стоит первым, а не
// вторым, как раньше.
...
// Порядок разделов — снизу вверх, как здание и обходят: подвал, этажи,
// мансарда (решение пользователя 11.09.2026). Раньше подземные стояли вторыми,
