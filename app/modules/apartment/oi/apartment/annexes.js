// Пристройки квартиры — таблицей, как в техпаспорте.
//
// Перенесено из карточки гражданского здания (civil/oi/building/annexes.js) по
// указанию пользователя 10.09.2026: «блок лоджии, балконы и террасы замени на
// блок Пристройки из карточки Гражданское здание, чтоб таким же был». Код
// скопирован, а не импортирован: modules/* → modules/* запрещено (CLAUDE.md,
// «Архитектурные правила проекта»).
//
// Требование, из которого блок вырос (09.09.2026): «вбивать таблицей — литеры
// ж, ж1, ж2, выбирать, что это (лоджия, балкон, веранда, терраса или иное),
// новое добавляем плюсиком, материалы как и надо — фундамент, стены, кровля, и
...
//
// ОТЛИЧИЕ ОТ ГРАЖДАНСКОГО: у квартиры данные карточки живут в `oi.apartment`, а
// не в самой литере — там же, где лежали лоджии и балконы. Поэтому список
// берётся через host(), а не прямо из oi. Подсказка литеры при этом читает
// `oi.letter`: литера у объекта имущества, а не у его квартирной части.
...
// Хранилище квартирной части объекта имущества. Создаётся по обращению: у
// записей, которых ещё не касались, `oi.apartment` может не быть вовсе.
const host = (oi) => (oi.apartment = oi.apartment || {});

export const annexesOf = (oi) => {
  const h = oi && oi.apartment;
...
export function removeAnnex(oi, id) {
  host(oi).annexList = annexesOf(oi).filter((a) => a.id !== id);
}
...
// Подсказка в поле литеры — от литеры самого объекта имущества, строчной буквой
// и с номером: в техпаспорте пристройки литеры «Ж» подписаны «ж», «ж1», «ж2»
// (требование пользователя 09.09.2026). Это именно подсказка — значение
// подставляет человек.
//
// Литера бывает составной («Г, Г1», «А1»), поэтому берём из неё первую
