...
// Этажи САМОЙ КВАРТИРЫ, а не дома: квартиры бывают двухуровневыми и больше
// (указание пользователя 10.09.2026 — «не путай его с этажностью самого
// здания»). Этажность дома — отдельное поле в блоке 01, рядом с этажом и
// количеством комнат.
//
// Поле стоит над развёрткой, как «Количество этажей» в карточке гражданского
// здания: оно и задаёт её состав. Привязка прежняя — data-apt-storeys, его
// обработчик пишет apt.storeys и oi.floors (ctrl.js), — поэтому переезд поля из
// блока 01 ничего не ломает в уже введённом.
export function floorsCountField(oi) {
  ...
  return `<div class="field"><label>Количество этажей в квартире</label>
<input class="input" data-apt-storeys value="${esc(apt.storeys || '1')}"
  inputmode="numeric" min="1" max="30"
  title="Этажи самой квартиры — двухуровневая и больше (до 30). Этажность дома задаётся отдельным полем в блоке 01">
