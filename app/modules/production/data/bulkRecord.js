...
export function buildBulkRecord(id, i, p) {
  ...
  rec.oi.push(buildingOi(id, i, p, 'А', {
    name: ['Производственный цех', 'Склад готовой продукции', 'Ремонтный бокс', 'Котельная'][i % 4],
    catClass: 'Производственно-складское',
    photoShare: 0.8,
  }));

  if (p.metrics.oiCount > 1) rec.oi.push(landOi(id, i, p, 1, { area: p.metrics.area * 2.5 }));
  if (rec.complex) rec.oi.push(movableOi(id, i, p, 1, 'МЕХ'));
  return rec;
}
