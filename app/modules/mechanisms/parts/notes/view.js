...
// У этого модуля нет ни одного вида ОИ (см. records.js: oiCards/oiTypes
// пустые), поэтому в отличие от production здесь нет метаданных карточки ОИ
// (../../oi/registry.js попросту не существует) и нет функции oiLabel —
// группа rec.oi ниже гарантированно пуста, guard оставлен на случай, если
// это когда-нибудь изменится.
export function drawerNotesHTML(rec, ui) {
  const groups = [
    { scope: 'oc', label: 'ОЦ', notes: rec.notes || [] },
    ...(rec.oi || []).map((o) => ({ scope: o.id, label: o.name || o.id, notes: o.notes || [] })),
  ];
