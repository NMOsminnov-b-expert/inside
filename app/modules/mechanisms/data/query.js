...
// Копия структуры остальных модулей (см. app/README.md, контракты) — сама
// логика проще, потому что у записи нет площадей, объектов имущества и кода
// ЕНИ (см. records.js — summarize()).
...
// skip — имя пропускаемого критерия (для фасетных счётчиков: фасет не должен
// ограничивать сам себя).
export function matches(s, f, skip) {
  if (!f) return true;

  if (skip !== 'status' && !inList(f.status, s.status)) return false;
  // У этого модуля нет кода ЕНИ (см. records.js), а «регион» у остальных
  // модулей выводится именно из его первых цифр (eniRegion) — выводить
  // регион неоткуда. Поэтому запись этого типа ОЦ не относится ни к одному
  // региону: если фильтр меню сужен по региону, такие записи из выборки
  // выпадают (а не остаются в ней независимо от выбора) — иначе они путались
  // бы под любым выбранным регионом сразу.
  if (skip !== 'region' && f.region && f.region.length) return false;
...
export function sortRows(rows, sort) {
  ...
  const base = CMP[key] || CMP.updatedAt;
...
const FLAG_KEYS = ['pendingNotes'];

function emptyFacets() {
  // region — обязательный ключ формы фасетов (её ждёт общий pages/ocMenu/query.js,
  // который сливает фасеты всех типов ОЦ и слепо делает Object.keys(f.region) для
  // каждого). У этого модуля региона нет и не будет (нет кода ЕНИ, откуда его
  // выводят остальные модули — см. matches() выше), поэтому бакет всегда пуст, а
  // не отсутствует.
...
// Локатор: у этого типа ОЦ нет кода ЕНИ и литер — только адрес/учреждение.
