...
def run(t):
    ...
    # --- 4. поле с разделами (класс ОИ) ---
    # Поле и перечень 09.09.2026 переименованы из «Категории ОИ» в «Класс ОИ»
    # (решение пользователя по блоку 01). Сценарий ищет справочник по подписи
    # слота, поэтому подпись здесь обязана совпадать с карточкой.
    if t.ck(_open_dict(t, 'civil', 'Литера', 'Класс ОИ'), 'не открыл справочник классов ОИ'):
        ...
        t.ck(len(og) == 1, 'в карточке гражданского здания разделы «Класса ОИ»: %s' % og)
        t.ck(og and 'дминистративн' in og[0],
             'в карточке гражданского здания не тот раздел классов: %s' % og)
