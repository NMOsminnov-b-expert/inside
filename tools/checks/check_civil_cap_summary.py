...
Сценарий держит:
  * строки: литеры-строения и зоны литеры с площадью по внутреннему обмеру,
    К, классом, площадью × К, состоянием, площадью × состояние и площадью ×
    состояние × К — колонки сводной таблицы методологии;
  * итог: площади суммой, К — средневзвешенный по площади строк с классом;
    он же крупно над таблицей — «Средневзвешенная капитальность»;
...
def run(t):
    ...
    avg = pg.inner_text('#q-capsum [data-cs-avgk] .cs-avg-v').strip()
    t.ck(avg == foot[2], 'средневзвешенная капитальность над таблицей не та, что в итоге: %s / %s' % (avg, foot[2]))
    # Состояние взвешенное (площадь × ранг) — колонка методологии.
    t.ck(abs(num(a[3]) * 2 - num(a[8])) < 0.05, 'площадь × состояние у литеры А не сходится: %r' % a)
