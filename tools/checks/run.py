...
def main():
    ...
    # Потоков — не больше ПОЛОВИНЫ ядер. Браузер не считающий процесс: на каждую
    # страницу он поднимает свой renderer, и при потоке на каждое ядро машина
    # уходит в перегрузку. Проявляется это не замедлением, а ложными падениями:
    # локатор ждёт элемент 30 секунд и не дожидается, потому что отрисовка не
    # получает процессорного времени (ловил такие провалы 04.09.2026 — от
    # прогона к прогону падали разные проверки).
