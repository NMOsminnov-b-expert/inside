# -*- coding: utf-8 -*-
"""Гражданское здание: зоны литеры.
...
Сценарий держит:
  * «Разбить литеру на зоны» делает две зоны: первая — то, что было у литеры
    (тип, признаки, площадь по внутреннему обмеру, высота), вторая — пустая;
  * у зоны свой тип, свои признаки, своя высота — и свой класс;
  * сверка «Σ площадь зон … из … м²»: «не хватает» / «сходится» / «лишние»;
...
def run(t):
    ...
    t.ck('Гражданская' in kinds and 'Производственно-складская' in kinds and 'зон: 2' in kinds,
