...
Сценарий держит:
  ...
  * после перезагрузки на медленном сервере (Live Server) меню «+» открывается:
    возврат файлов раньше перерисовывал экран поддельным переходом, карточка
    монтировалась дважды, и у «+» было два обработчика — открыть и закрыть
    (замечание пользователя 25.09.2026: «выпадающего меню на + нет»);
...
def run(t):
    ...
    # --- перезагрузка: стили отдаются с задержкой, как у медленного сервера ---------
    import time
    pg.route('**/*.css', lambda route: (time.sleep(0.3), route.continue_()))
    pg.reload()
    t.wait_for('.viewer')
    ...
    pg.unroute('**/*.css')
    pg.locator('.vtab-plus').click()
    t.ck(pg.locator('.vtab-add.open').count() == 1, 'после перезагрузки меню «+» не открывается')
    pg.locator('.vtab-plus').click()      # повторный щелчок закрывает меню (Esc закрыл бы просмотрщик)
