...
def run(t):
    ...
    # Литера может быть привязана к участку с самого начала: в исходных данных
    # civil строения стоят на своём участке (09.09.2026), иначе участок
    # показывался пустым. Сценарию нужна открепленная литера — открепляем сами,
    # а не рассчитываем на то, каким окажется набор данных.
    if _land_of(pg, letter):
        pg.evaluate(DRAG, [letter, '.viewer', 80])
        t.wait(300)
        _confirm(pg, t.wait)
    t.ck(_land_of(pg, letter) == '', 'подготовка: литеру не удалось открепить')
