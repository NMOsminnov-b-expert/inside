...
def run(t):
    ...
    # Карточка литеры приезжает лениво (import), поэтому ждём и строку
    # перечня, и само поле ПО ФАКТУ. С отсчётом времени проверка падала на
    # занятой машине — «в карточке литеры нет поля названия», а следом пустым
    # оказывался и лог (флак 08.09.2026, пятый случай того же класса).
    t.open('#/oc/civil/oc-cv-1', wait='tr[data-open-oi]')
    pg.locator('tr[data-open-oi]').first.click()
    t.wait_for('[data-oi-name]')
    t.wait(200)
