...
# В гражданском здание одно: гражданское оно или производственное, указывают
# внутри карточки (решение пользователя 23.09.2026).
REALTY_CIVIL = ['Земельный участок', 'Квартира', 'Жилой дом', 'Здание', 'Прочее строение']
...
def run(t, part=None):
    ...
    for label, route in ({part: ROUTES[part]} if part in ROUTES else
        ...
        missing = [x for x in (REALTY_CIVIL if 'civil' in route else REALTY) if x not in items]
...
#
# От вида литеры зависит состав ОСТАЛЬНЫХ полей карточки: у гражданской свои
# признаки класса, у производственно-складской — свои и блок «Доп параметры».
# Человек менял вид и не видел, что вместе с ним со страницы ушли заполненные
# поля — они просто перестают показываться, данные остаются в записи. До
# 23.09.2026 состав решало назначение по техпаспорту; теперь вид литеры по фото
# (решение пользователя), и предупреждение переехало к нему.
#
...
def check_hidden_fields_warning(t):
    ...
    pg.wait_for_selector('[data-lit-kind="prod"]', timeout=15000)
    t.wait(700)

    # У гражданской литеры заполнены признаки класса — при смене вида на
    # производственный они уходят, значит нужен диалог.
    pg.click('[data-lit-kind="prod"]')
    t.wait(800)

    if t.ck(pg.locator('.modal-head').count() == 1,
            'смена вида литеры прошла без предупреждения о скрытых полях'):
    ...
    pg.click('[data-lit-kind="prod"]')
    pg.locator('[data-lit-kind="prod"]').dispatch_event('click')
    t.wait(800)
