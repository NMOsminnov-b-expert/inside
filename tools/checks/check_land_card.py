...
def run(t):
    ...
    # --- 6. адрес и координаты участка ---
    #
    # Крупная зона и микрорайон уехали в объект оценки (заметки команды
    # 05.09.2026): они общие для записи. У участка вместо них свой адрес —
    # улица и дом, как у остальных ОИ, и состав полей больше не зависит от того,
    # в каком городе объект.
    t.ck(pg.locator('[data-land-gps]').count() == 1, 'нет поля координат')
    t.ck(pg.locator('[data-oi-street]').count() == 1, 'нет улицы участка')
    t.ck(pg.locator('[data-oi-house]').count() == 1, 'нет дома участка')
    t.ck(pg.locator('[data-land-zone]').count() == 0,
         'крупная зона осталась в карточке участка')
    t.ck(pg.locator('[data-land-location]').count() == 1,
         'нет поля «Расположение в районе»')

    # В крупном городе набор полей тот же самый.
    _open_land(t, '#/oc/civil/oc-cv-1')
    t.ck(pg.locator('[data-land-zone]').count() == 0
         and pg.locator('[data-land-microdistrict]').count() == 0,
         'в Бишкеке вернулись крупная зона и микрорайон')
    t.ck(pg.locator('[data-land-location]').count() == 1,
         'в крупном городе пропало «Расположение в районе»')
    t.ck(pg.locator('[data-oi-street]').count() == 1,
         'в крупном городе у участка нет своей улицы')
    ...
    # Типов ОЦ стало шесть: с 17.09.2026 в реестре есть «Транспортные средства»
    # (движимое имущество как самостоятельный объект оценки).
    ...
    # поэтому ждём появления карточки нового типа, а не времени. Ждать .card
    # мало: сама карточка появляется раньше перечня ОИ, и при занятой машине
    # литеры пересчитывались на пустом перечне — «стало 0 вместо 4», а
    # следующий шаг не находил уже поле формы (флак 08.09.2026). Ждём именно
    # то состояние, которое проверяем; не дождёмся — проверка ниже упадёт по
    # делу, а не по случайности.
    t.wait_for('.card')
