...
class Tester:
    ...
    def open(self, route='', wait='.card, .arc, .reg-thead', timeout=9000):
        # '' → '#/': переход с «#/oc/…» (или любого другого маршрута) на голый
        # base-URL без решётки — это не переход внутри страницы, а обычная
        # загрузка документа, и она стирает состояние приложения (архив,
        # только что сделанные правки) — тот же эффект, что при F5. С '#/' это
        # переход только по фрагменту, документ не перезагружается.
        if not route:
            route = '#/'
