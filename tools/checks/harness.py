...
def run_one(mod, part=None, headless=True, browser_=None):
    ...
    def go(br):
        ...
        # Данные модулей переживают перезагрузку через localStorage
        # (kernel/persist.js, 09.09.2026). Для сценария это чужое состояние:
        # правки предыдущего прогона доставались бы следующему, и порядок
        # запуска начал бы влиять на результат. Чистим на каждой странице —
        # сценарий всегда стартует с засева.
