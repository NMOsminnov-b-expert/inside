...
def run(t):
    ...
    # --- 4. карточка квартиры: мультивыбор материалов и отопления ----------
    #
    # Материалы конструктива у квартиры есть с 11.09.2026 (решение
    # пользователя: конструктив и состояние квартиры описываются здесь же).
    # Прежде их не было, и сценарий сторожил обратное — то ожидание отменено
    # более поздним решением. Сторожим другое: поля на месте. Как открывается
    # сам список, проверено выше на литере — там те же поля и тот же код.
    t.open('#/oc/apartment/oc-ap-1', wait='tr[data-open-oi]')
    pg.locator('tr[data-open-oi]').first.click()
    t.wait_for('[data-heat-field]')
    pg.locator('#q-struct').scroll_into_view_if_needed()

    fields = pg.evaluate("""() => [...document.querySelectorAll('#q-struct [data-struct-field]')]
        .map((f) => f.dataset.structField)""")
    t.ck(len(fields) >= 4,
         'в карточке квартиры меньше четырёх материалов конструктива: %s' % fields)
