...
def run(t):
    ...
    # --- 1. карточка ОИ: земельный участок ---
    #
    # Единственная карточка объекта имущества, где координаты ВВОДЯТ. У строения
    # своих координат больше нет: местоположение целиком у объекта оценки, блок
    # «Местоположение» (решение пользователя 09.09.2026).
    if _open_oi(t, '#/oc/land-plot/oc-lp-1/oi/oi-lp1-l1', '[data-land-gps]'):
        _check_field(t, '[data-land-gps]')

    # --- 2. карточка ОИ: квартира — координаты записи, только для чтения ---
    #
    # Формат здесь проверять нечего: с 11.09.2026 улица, дом и координаты
    # квартиры показывают адрес записи и правятся в объекте оценки. Сторожим
    # именно это — иначе вернётся второе место ввода одного значения, из-за
    # которого адрес в отчёте и расходился.
    if _open_oi(t, '#/oc/residential-house/oc-rh-1/oi/oi-b', '[data-oi-gps]'):
        fld = pg.locator('[data-oi-gps]')
        t.ck(fld.get_attribute('readonly') is not None,
             'координаты квартиры правятся в карточке ОИ, а должны приходить из записи')
        t.ck(bool(fld.input_value().strip()),
             'координаты квартиры пусты, хотя у записи они заданы')
