...
export default [
...

  {
    ...
    rules: {
      ...
      'no-dupe-keys': 'error',
      // Начальное значение переменной, которое дальше в любом случае
      // перезаписывается, правило считает лишним. В макете это защита: ветка
      // без присваивания дала бы undefined в разметке, а не пустую строку.
      // Ошибкой не считаем.
